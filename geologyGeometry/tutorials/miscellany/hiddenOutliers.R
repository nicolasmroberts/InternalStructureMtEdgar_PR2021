...
# This tutorial explains how we generated the synthetic outlier examples used 
# in our orientation and ellipsoid tutorials. Because the code uses random 
# numbers, the results are different every time. The outlier is always similar 
# to the blue data in one aspect and to the green data in another.
...
### LINE-IN-PLANE ORIENTATIONS ###

# First we pick lines l1, p1, l2, p2 such that (A) l1 and p1 are 
# perpendicular, (B) l2 and p2 are perpendicular, and (C) l1 and p2 are 
# perpendicular. That's the key.
synthL1 <- lineUniform()
synthP1 <- lineUniform()
synthP1 <- rayNormalized(synthP1 - dot(synthP1, synthL1) * synthL1)
synthP2 <- lineUniform()
synthP2 <- rayNormalized(synthP2 - dot(synthP2, synthL1) * synthL1)
synthL2 <- lineUniform()
synthL2 <- rayNormalized(synthL2 - dot(synthL2, synthP2) * synthP2)
...
# Color roughly the first half blue, the second half green, and the last one 
# red.
...
# Here is the equal-area plot. The red circle should be similar to the blue 
# circles. The red square should be similar to the green squares.
...
# Here is the equal-volume plot. Depending on how the random numbers went, the 
# red point might or might not be obvious as an outlier.
oriEqualVolumePlot(synthRs, group=oriLineInPlaneGroup, colors=synthColors)

# The quickest way to try again is to copy and paste all of this code into the 
# RStudio Console, press Return, inspect the results, press up-arrow and 
# Return in the Console to run the code again, inspect the results, and so on.
...
# Make a swath of 30 octahedral shear strains and a swath of 30 Lode's 
# parameters.
...
# Make the outlier by mating the first orientation with the last shape, with a 
# small bit of noise.
...
# The red orientation is similar to the blue orientations, but the red shape 
# is similar to the green shapes. Depending on your particular random example, 
# the outlier might or might not be obvious in the vector pair plots.
...
# Here is the fastest way to try it again: Copy and paste all of those lines 
# of code into the R console, and press Return. Inspect the results. To do it 
# again, click in the R console, press the up-arrow key, and press Return.



### CONCLUSION ###

# When your data are high-dimensional, you can't view all of the dimensions at 
# once. You can rig up a 'hidden outlier' example by making it similar to half 
# of the data in some dimensions and the other half of the data in the other 
# dimensions.
