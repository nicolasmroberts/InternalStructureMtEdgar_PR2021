...
# This tutorial continues our ellipsoid inference tutorials. We apply exactly 
# the same methods to other data sets.
...
# We're using data sets from the Huemul pluton, Chile (Garibaldi et al., in 
# prep). There is one anisotropy of magnetic susceptibility (AMS) ellipsoid. 
# Then there are 'clast' ellipsoids produced by X-ray computed tomography 
# (XRCT) on the same rocks. Some of the clasts have 'medium' composition 
# (between felsic and mafic). Other clasts are actually voids in the rock.

# Load one AMS ellipsoid from a file.
...
# In brief, here are the same four hypothesis tests for clasts of 'medium' 
# composition in the same sample. Sometimes the MM- and S-type inferences 
...
# Here are the same four hypothesis tests for voids (air pockets) in the same 
# sample.
...
# In all of these examples, we reject the hypothesis that the AMS ellipsoid is 
# the mean of the population from which the clast ellipsoids were drawn. So it 
# appears that we cannot use AMS as a simple, easy proxy for XRCT.
