...
geoRotationsFromDikePaleomag <- function(pole, dir, tmv) {
  ...
  if (length(us) >= 1) {
    ...
    rs <- lapply(sigmas, function(sigma) {t(mMat) %*% rotMatrixAboutX(sigma) %*% tMat})
    absZs <- lapply(rs, function(r) abs((r %*% pole)[[3]]))
...
geoEllipsoidDataFromAGICOFile <- function(fileName, meanSuscepts, doNormalize=FALSE, separator=NULL) {
  ...
  dataFrame <- cbind(dataFrame, geoSemiaxisDataFromAGICOData(dataFrame, meanSuscepts, doNormalize))
  # Build the rotation field.
  p1s <- lapply(1:nrow(dataFrame), function(i) geoCartesianFromTrendPlungeDeg(c(dataFrame$K1dec[[i]], dataFrame$K1inc[[i]])))
  p2s <- lapply(1:nrow(dataFrame), function(i) geoCartesianFromTrendPlungeDeg(c(dataFrame$K2dec[[i]], dataFrame$K2inc[[i]])))
  dataFrame$rotation <- lapply(1:nrow(dataFrame),
