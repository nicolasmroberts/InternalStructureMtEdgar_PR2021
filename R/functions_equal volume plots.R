...
plotEllipsoid = function(rots, as, centers=replicate(length(rots), c(0, 0, 0), simplify=FALSE), numNonAdapt=4, ellOpacity = 1, ellColor = "#e1e1e1", axisColor = c("black", "black","black")){
    ...
    sphere = lapply(sphere, function(tri) lapply(tri, function(S) S * 500))
    ...
    eastAxis = data.frame("x" = c(1000,-1000), "y" = c(0,0), "z" = c(0,0))
    northAxis = data.frame("x" = c(0,0), "y" = c(1000,-1000), "z" = c(0,0))
    upAxis = data.frame("x" = c(0,0), "y" = c(0,0), "z" = c(1000,-1000))
    
    # pre-define the 3D annotations for east, north, and up
    eastAnn <- list(x = 1000 + 200,
                    y = 0,
                    z = 0,
                    xanchor = 'middle',
                    text = "East",
                    font = list(family = 'Helvetica',size = 30, color = 'rgba(0,0,0,1)'),
                    showarrow = FALSE)
    ...
    p <- plot_ly() %>%
        add_trace(data = triangles, x = ~x, y = ~y, z = ~z, i = ~i, j = ~j, k = ~k,
                  type = "mesh3d", 
                  facecolor = rep(I(ellColor), nrow(triangles)), 
                  opacity = ellOpacity,
                  lighting = list(specular = 0.8)) %>%
        #lightposition = list(x = 10, y = 10, z = 10))  %>%
        add_trace(data = eastAxis, name = "East", x= ~x, y = ~y, z = ~z, type = "scatter3d", mode = "lines+markers", marker = list(color = I(axisColor[1]), size = 4), line = list(color = I(axisColor[1]),width = 5), opacity = 1,
                  inherit = FALSE) %>%
        add_trace(data = northAxis, name = "North", x= ~x, y = ~y, z = ~z, type = "scatter3d",mode = "lines+markers", marker = list(color = I(axisColor[2]), size = 4), line = list(color = I(axisColor[2]),width = 5), opacity = 1,
